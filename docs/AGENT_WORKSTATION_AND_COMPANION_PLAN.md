...
> 状态：Desktop 0.1.80 已交付 P0/P1：单一官方桌宠、状态动作/道具、脱敏 workforce
> 协议，以及可收起的 2.5D Agent 办公室；真实 3D 主题、历史回放和全局调度仍在后续阶段。
> 决策日期：2026-08-14。
...
Hara 已从标识中的熊形轮廓建立一个统一角色母版。Desktop 0.1.80 收敛为**一个官方身份**，
不再把开发、设计、研究等职责做成一排长相接近的选择项。能力差异属于 Agent 的实时职责，
由工位、工具和状态动作表达，不应变成多个常驻虚构人物。

- 当前交付是一张经过透明边缘和小尺寸检查的官方静态角色图。Desktop 用跑步机与锤子、
  举牌、咖啡、交付旗和检修工具表达工作、等待、暂停、完成与受阻；这些道具固定在角色
  身体或脚下，不是漂浮在桌面的装饰，也不冒充完整 Codex/Hara v2 动画包。
