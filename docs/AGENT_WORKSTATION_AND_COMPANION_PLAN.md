# Hara 协作工位与桌宠统一状态方案

> 状态：已交付单一官方 v2 动画桌宠、统一状态播放器、脱敏 workforce 协议，以及同一
> 聊天扩展屏内可切换的 WebGL、低功耗战术平面和成员列表。2026-08-22 的当前工作树进一步
> 加入注册 Agent 常驻名单、聊天顶部直接入口、办公室切换、上帝视角和点击 Agent 对话；
> 历史回放与自定义 3D 资产仍在后续阶段。
> 决策日期：2026-08-14。
> 适用范围：Hara CLI / Serve、Hara Desktop 右侧扩展屏、Desktop 桌宠。

## 1. 产品决定

Hara 应吸收旧 Mission Control 像素办公室中“让 Agent 的真实工作状态可见、可理解、可
进入”的价值，但不直接嵌入或复制旧应用。

首个产品形态命名为 **协作工位**（内部 surface kind 可使用 `workforce`）：

- 它是当前聊天或项目会话拥有的原生右侧扩展屏标签，可收起、关闭、调宽和聚焦。
- 左侧聊天仍是任务的控制入口；点击工位中的角色回到精确会话、审批或结果。
- 注册 Agent 是可选择的常驻成员，空闲时明确显示“可对话”；只有 Serve 生命周期事件能
  把它们标为排队、工作、等待或阻塞，不能用随机动画伪造执行。
- Agent 选择器与办公室人数直接出现在聊天顶部，即使尚未建立会话也可选择 Agent 或进入
  办公室；办公室不再依赖用户先去能力/技能目录发现。
- 桌宠是同一份活动状态在扩展屏收起、Desktop 失焦或窗口关闭后的系统级投影。
- 演示文稿、表格和文档统一归入“交付物”；`Office` 只保留为内部能力协议名称，避免与
  “Agent 办公室”的用户心智混淆。
- 初版不增加常驻左侧导航；只有多会话全局调度需求被真实使用验证后，才增加全局任务中心。

## 2. 旧 Mission Control 原型的取舍

### 可以学习

- 工位、走动、打字、等待气泡、出生/离场、交付和短暂庆祝等可读状态。
- 父 Agent 与子 Agent 的空间关系，以及角色进入、工作、等待和结束的连续性。
- 猫、休息区、咖啡区等低干扰趣味元素，但必须服务于真实状态或环境氛围。
- 稳定座位、可点击角色和状态检查器的空间记忆。

### 不直接复用

- 当前 `OfficeView` 硬编码的角色名单和 `online -> working` 粗略映射。
- 与 Hara 会话、审批、恢复和权限边界无关的独立 Dashboard 数据源。
- 整个页面 iframe / WebView 嵌套、每个实例独立 60fps 循环和随机重排座位。
- 未确认许可证与来源的角色、家具和音效资产。
- 展示提示词、思维链、完整工具参数、命令、路径、输出或凭据的状态气泡。

File、Computer、Browser、Design、Office 等仍是**能力区域**，不是永远在线的虚构 Agent。
只有 Agent 目录里真实注册的身份可以作为空闲常驻成员；临时子 Agent 则只有 Serve 报告
真实执行者时才进入场景。

### GOD 的补充参考结论

`XiaoLuoLYG/GOD`（Apache-2.0）不是可直接嵌入的 3D 办公室，但它补足了 Mission
Control 缺少的运行记录模型：

- 把 Experiment、正在运行的 Runtime 和 Replay 明确分开，历史视图不反向成为当前状态真源。
- 用带 schema/capability 的 dataset catalog 描述 Agent 快照、环境快照、事件和指标，再按
  step 返回组合后的 replay bundle。
- 角色位置、资源包、时间线、定向 Ask / Intervene 各自有明确数据边界；地图和 Agent
  资源可以导入导出。
- Phaser 像素场景和 Deck.gl 地图证明大量角色应由单一场景引擎批量绘制，而不是每个角色
  建一个 DOM/WebView。
- 俯视地图、语义缩放、可拾取角色、左右信息面板和定向对话证明“大量角色总览”和“点人
  进入精确交互”可以共用一个稳定场景，而不是另建模拟器。

Hara 学习上述“快照 + 回放 + 资源包”边界，但不引入 GOD 的社会仿真运行时、地理地图、
完整人物状态或提示词检查器。Hara 的场景只消费脱敏的 `workforce_state`，Ask / Intervene
仍回到 owner 会话和既有审批边界。

## 3. 一个状态真源

将现有桌宠专用活动状态提升为共享的 `AgentActivityLedger`。三个界面只做选择与投影：

```text
Hara CLI / Serve
  └─ event.workforce_state.v1（有序、可恢复、脱敏快照）
       └─ Desktop AgentActivityLedger
            ├─ 当前会话 → 右侧协作工位
            ├─ 全部会话 → 后续全局任务中心
            └─ 最高优先级一项 → 桌宠
```

现有 `event.task_state` 继续负责根任务生命周期。新增事件补足根 Agent、子 Agent 和外部
执行者的并发关系，不能通过解析聊天文案或工具返回值推断。

建议首版协议：

```ts
interface WorkforceStateEventV1 {
  version: 1;
  streamId: string;
  sequence: number;
  sessionId: string;
  taskId: string;
  turnId: string;
  mode: "snapshot";
  actors: Array<{
    actorId: string;
    parentActorId?: string;
    kind: "root" | "subagent" | "external";
    role?: string;
    capability: "orchestration" | "files" | "code" | "browser" |
      "research" | "design" | "office" | "communication" | "other";
    state: "queued" | "working" | "waiting" | "paused" | "blocked" |
      "completed" | "failed" | "cancelled";
    activity: "planning" | "reading" | "writing" | "running" |
      "reviewing" | "awaiting_approval" | "delivering" | "idle";
    startedAt: string;
    updatedAt: string;
    endedAt?: string;
  }>;
}
```

协议约束：

- `sequence` 在 `streamId` 内严格递增；重连和 `session.resume` 必须恢复当前快照。
- 每个会话最多返回 24 个可视角色，更多执行者聚合为一组，避免 UI 和事件风暴。
- 工具名在 Serve 侧映射为有限的 capability / activity 枚举。
- 事件不得包含任务原文、思维链、完整工具参数、命令、绝对路径、文件内容、输出或凭据。
- 当前正在执行的一轮保持稳定；协议升级、重连和旧引擎降级必须有兼容测试。

## 4. 状态到场景的确定性映射

| 真实状态 | 工位表现 | 桌宠表现 |
|---|---|---|
| 根任务规划 | 中央调度桌或白板 | 处理中 |
| 子 Agent 排队 | 入口或预留座位 | 不抢占更高优先级状态 |
| 文件/研究 | 资料架或阅读工位 | 处理中 |
| 浏览器/计算机操作 | 显示器工位 | 处理中 |
| 设计 | 白板或画架 | 处理中 |
| PPT/文档/表格 | Office 能力工位 | 处理中 |
| 等待用户或审批 | 举手、琥珀提示和明确文字 | 需要你确认 |
| 暂停 | 咖啡或休息区 | 已安全暂停 |
| 阻塞/失败 | 红色修复区和明确原因类别 | 遇到问题 |
| 完成 | 短暂交付托盘/庆祝，随后淡出或待命 | 任务完成 |

动画只能由真实事件触发。没有真实 handoff、等待或子 Agent 事件时，不播放相应动画。
同一 `actorId` 的座位必须稳定，不能因每次刷新随机变化。

## 5. Desktop 交互

- `+` 菜单增加“协作工位”，默认绑定当前聊天/项目会话 owner。
- 标签头显示工作中、等待和阻塞数量；扩展屏关闭后完整归还聊天空间。
- 主体由场景与紧凑角色列表组成；场景负责理解，列表负责精确状态、键盘操作和无障碍。
- 点击角色只执行安全导航：打开精确会话、审批卡或已完成结果。
- 审批仍在现有聊天/审批面板完成；场景本身不获得新的权限。
- 当前会话和全局范围必须明确区分，禁止不同 Chat、Project、Task 或 Office owner 混用权限。

## 6. 桌宠协同

- 桌宠不维护第二套任务真相，只从共享 ledger 选择一个最高优先级活动。
- 优先级保持：等待审批 > 阻塞 > 暂停 > 完成 > 工作中 > 空闲。
- 桌宠点击后打开产生该状态的精确会话和任务，不跳到“最近活动”的其他项目。
- 常驻顶层窗口继续只显示脱敏状态标题，不显示客户内容、目标、路径或工具输出。
- 协作工位可展开查看多个角色；桌宠只表达一个当前最需要用户注意的状态。

## 7. 视觉、性能与可访问性

- 2.5D 回退继续使用 DOM/SVG 和经过验证的桌宠 spritesheet；3D 首版使用 Hara 自有的
  程序化角色、家具与能力道具，不下载外部模型或贴图。
- WebGL 场景按需加载并封顶 30fps；标签隐藏、离开视口或减弱动画时停止连续绘制。
- 支持 `prefers-reduced-motion` 和完全静态模式。
- 颜色之外同时使用图标和文字；角色列表具备键盘焦点、ARIA 标签和与场景等价的信息。
- 首版最多 24 个角色，只引入一个懒加载 Three.js renderer，不引入独立游戏运行时。
- 暂不抽 npm 包。只有 Desktop 与第二个真实消费者都需要该场景后，再评估
  `@nanhara/agent-office-scene`。

### 7.1 从 2D 工位到 3D 办公室

3D 是可行的表现层升级，不是新的 Agent 后端：

1. P1 已用 2D / 2.5D 验证真实状态、选择、稳定座位和性能预算。
2. Desktop 0.1.83 已在同一个 `workforce` 标签内加入懒加载 Three.js 主题；首版使用
   程序化 Hara 几何体，不新增 iframe、第二个桌面窗口或独立 Agent 后端。
3. 场景隐藏时停止动画和 GPU 绘制；可见时目标 30fps，低功耗模式 12–15fps，24 个角色
   以上聚合显示，并保留等价的 DOM 列表和 2D 降级路径。
4. 3D 角色只读取 `actorId / parentActorId / capability / state / activity`，不得读取提示词、
   文件、命令或思维链。位置由稳定哈希和确定性区域布局生成，重连后不能换座。
5. 只有 Desktop 和第二个真实前端共同消费并验证后，才抽取场景 npm 包；首版直接集成进
   Desktop，避免为了“可能复用”先制造版本和协议耦合。

### 7.2 Hara 官方桌宠角色线

Hara 已从标识中的熊形轮廓建立一个统一角色母版。Desktop 0.1.81 收敛为**一个官方身份**，
不再把开发、设计、研究等职责做成一排长相接近的选择项。能力差异属于 Agent 的实时职责，
由工位、工具和状态动作表达，不应变成多个常驻虚构人物。

- 当前交付是经过帧边界、透明背景、方向语义、连续性和小尺寸检查的官方 `8×11` v2 图集；
  Desktop 安装包直接内置它，默认桌宠不再依赖 `~/.codex/pets`。工作、行走保持连续动画，
  等待、暂停、完成与受阻只播放一次后停帧，空闲只偶尔呼吸，避免持续晃动。
- 0.1.79 已保存的十个旧官方 selector 会安全迁移到统一角色，不会造成空白桌宠。
- 图集的 SHA-256、尺寸与 v2 元数据由回归测试锁定；未来换图必须重新完成确定性裁切、
  方向盲测、透明残留检查和实际显示尺寸 QA，不能直接覆盖已验收资产。
- 未来 v2 图集体积显著增加时，继续只内置一个官方角色；其他角色属于用户明确安装或
  自定义生成的扩展包，不能把多套完整图集无预算地永久塞进安装包。
- 当前代码和站点历史注释曾把标识描述为 cat，而产品提议使用“熊”。批量生成前应在品牌
  资产说明中一次性确定物种、耳朵、眼睛、口鼻和身体比例，避免七个角色出现品牌分裂。

### 7.3 自定义桌宠与后端边界

自定义生成可以后置。成熟形态不是另做一套桌面业务后端，而是在现有市场能力旁增加一个
隔离的资产作业服务：

```text
Desktop 上传参考图 / 品牌约束
  -> Pet Job API（鉴权、额度、内容与授权声明）
  -> 异步 Image 生成 worker
  -> 确定性 atlas QA / 签名 / 版本化
  -> Hara Market 私有或公开包
  -> Desktop 再做本地校验后安装
```

作业服务只接触用户明确上传的视觉参考和角色说明，不接触会话历史、项目文件、提示词、
工具输出或 `AgentActivityLedger`。Agent 在线状态继续由本机 CLI / Serve 提供；即使资产服务
离线，已安装桌宠与协作工位也必须完整可用。初期可以由 Hara Market 承担目录、版本、权限
和下载，只有生成队列、GPU 成本和审核负载形成独立扩缩容需求时，再拆 `hara-pet-service`。

## 8. 实施阶段与门禁

### P0：协议与恢复

- Serve 发出有序、脱敏的 workforce 快照，覆盖根 Agent 和真实子 Agent。
- Desktop reducer 覆盖乱序、重复、断线、重连、resume、旧引擎和 24 人聚合。
- 安全测试证明敏感原文不会进入事件、日志、桌宠或系统通知。

### P1：当前会话协作工位

- 在现有 owner-scoped Extension Dock 中新增原生 `workforce` 标签。
- 先交付根 Agent、列表、确定性工位和工作/等待/阻塞/完成状态。
- 关闭、收起、调宽、聚焦和切换会话均不串状态。

### P2：真实子 Agent 与桌宠统一

- CLI 子 Agent runtime 暴露 queued / started / settled 生命周期。
- 子 Agent 出生、工作、等待、交付和离场全部由协议驱动。
- 桌宠改为共享 ledger selector，验证优先级和精确回跳。

### P3：多办公室指挥台（2026-08-22 已进入实现）

- 当前项目办公室优先放项目 Agent，再以全局 Agent 补足常驻成员；无注册项目的工作区也
  不再退化成只有 `main`。
- 聊天顶部直接显示可用 Agent 数和办公室入口；办公室提供上帝视角、Mission Control
  战术平面与成员列表，并支持前后切换办公室。
- 后续在真实多会话调度需求继续增加历史时间线；实时视图与回放仍必须分离。
- 全局视图默认仍不合并权限；进入角色时回到其 owner 会话。

### P4：主题和历史

- 增加经过许可审查的主题、家具、节庆和完成回放。
- 趣味元素不能掩盖等待、失败、权限和任务真实性。

### P5：3D 与自定义资产（3D 基础已于 0.1.83 交付）

- 已在同一 `workforce` surface 增加可关闭的上帝视角和战术平面 / 成员列表降级；聊天顶部
  提供常驻但低干扰的 Agent 人数与办公室入口，不新增独立产品导航或第二套后端。
- 将已经发布的 11 个官方角色逐步升级为通过全套 v2 QA 的动画包，按实际安装体积决定
  内置或市场按需分发。
- 仅在资产授权、内容审核、生成成本、私有可见性、删除与版本回滚都闭环后开放用户生成。

## 9. 首版验收

首版只有同时满足以下条件才算完成：

1. 用户在空白聊天和已有会话都能看到 Agent 名单规模，并能从当前聊天打开和关闭协作工位；
   聊天和扩展屏空间行为与其他标签一致。
2. 根任务与至少一个真实子 Agent 的排队、工作、等待、完成和取消状态可重放验证。
3. 重连后角色、父子关系和状态恢复一致，不出现幽灵角色或随机换座。
4. 点击等待角色进入精确审批；点击完成角色进入精确会话或结果。
5. 桌宠与工位对同一活动给出一致状态，且不会泄露任务内容或凭据。
6. 隐藏标签不持续高频绘制；减弱动画、键盘和屏幕阅读器路径可用。
7. 当前项目、全局大厅和其他项目办公室可切换；点击注册 Agent 必须打开该身份独立历史，
   不得复用其他 Agent 的会话上下文。

## 10. DeepSeek Harness 对照审计与插件化边界（2026-08-17）

当前能力不能笼统描述为“已经引入 DeepSeek Harness”，准确状态如下：

| 能力 | Hara 当前状态 | 下一步 |
|---|---|---|
| DeepSeek Responses API 与思考档位 | CLI 0.148.0 已支持 Flash / Pro 的 Responses 路由，以及 `off / low / high / max` 的安全映射；无状态历史、语义 SSE、工具调用和 reasoning continuation 已覆盖 | 保持 provider contract 与回归，不在 Desktop 暴露私有思维链 |
| 本机子 Agent | 已有 provider-neutral `SubagentRuntime`、FIFO 有界队列、并发限制、取消、结算和生命周期事件；当前真实 provider 为 `native-readonly` | 把 provider 注册、能力声明和错误类型收敛成稳定 seam |
| Agent 办公室状态 | Serve 已投影有序、脱敏的 `event.workforce_state`；Desktop 2.5D 场景消费真实根 Agent 与子 Agent 状态 | 增加稳定多级谱系、可继续 child epoch 和历史回放事件 |
| Harness 多提供方机制 | 尚未引入 | 依次增加 Hara in-process、Codex app-server、Claude/ACP 等可选 adapter；不把外部实现硬编码进主循环 |
| 可继续子会话 | 尚未引入 | 设计稳定 child session id、直接父级鉴权、follow-up FIFO、interrupt、child-first settlement 与冷恢复 |
| 真 3D 办公室 | Desktop 0.1.83 已预装懒加载 WebGL renderer，0.1.84 增加 Workbench 与能力目录直达入口；WebGL 不可用时保留 2.5D/列表降级 | 在真实使用数据基础上继续优化程序化角色、空间与性能，不另建 Agent 后端 |

### 10.1 学习 Harness，但不照搬整个 Cordis 运行时

Hara 应学习 Harness 的三项结构，而不是把另一个完整产品运行时塞进 CLI：

1. **完整能力缝隙**：每项能力同时定义 Service Definition、一个或多个 Provider 和 Consumer；
   缺少任一角色都不称为插件能力。
2. **会话是真源**：任何会进入下一轮模型上下文的 child 消息、上报和结算都必须可从持久化
   会话事件恢复；办公室只消费投影，永远不能反向成为执行状态真源。
3. **可继续 child 的所有权**：稳定 child id、直接父级权限、唯一 FIFO inbox、冷恢复和
   child-first settlement 属于 runtime；UI、桌宠和 3D renderer 不参与这些判定。

保持 Hara 默认执行路径简单：没有开启多 Agent 执行时仍是一个 root Agent、一个会话和一条
任务状态链。注册 Agent 可以作为空闲、可点击的身份常驻；只有模型真实调用 subagent 工具
后才加载 provider、创建 child，并把临时协作者标为真实工作状态。

### 10.2 “万物皆插件”在办公室中的落地

Agent 办公室是**聊天内置工作模式**，不是用户需要安装或去技能中心寻找的技能。其内部仍
保持插件式边界，但插件化的是 renderer 和数据契约，不是产品入口，更不是 iframe：

```text
CLI orchestration capability
  = Subagent Service Definition + named Providers + model-tool Consumer
        -> Serve workforce projection plugin（有序、脱敏、可恢复）
             -> Desktop Agent Office surface plugin
                  -> WebGL 3D renderer（当前默认、按需加载）
                  -> 2.5D renderer（低功耗降级）
                  -> accessible list renderer（永久保留）
```

- Desktop surface 通过受控 contribution 注册标签、图标、命令和 renderer，不直接 import
  provider，也不能读取提示词、思维链、工具参数、文件路径或输出。
- 3D 只是同一份 `workforce` 快照的 renderer 插件；切换主题不能创建第二套 Agent 后端或
  第二套状态机。
- `reasoningEffort` 最多显示为有限的“思考强度”配置或 `planning` 状态；绝不展示
  reasoning 文本，也不把动画强度伪装成模型正在做的具体推理。
- 插件不可用、GPU 受限或用户启用减弱动态效果时，自动回退到 2.5D / 列表视图；任务执行
  本身不受影响。

### 10.3 下一阶段最小纵切

1. 定义 `SubagentProviderV1`：命名注册、能力声明、one-shot start、取消和 typed settlement；
   以现有 `native-readonly` 作为第一个 provider，不先引入完整 Cordis。
2. 为真实 child 增加稳定 `parentActorId / depth / providerKind / epoch` 的脱敏 v2 投影，保留
   v1 降级，并对乱序、重连、取消和 child-first 结算做端到端测试。
3. 将 `WorkforceSurface` 注册路径改为预安装 surface contribution；UI 与当前 owner、审批和
   会话权限保持不变。
4. 继续验收现有按可见性暂停的 Three.js renderer，以 24 个角色、30fps 上限、低功耗模式和
   静态降级作为持续发布门禁；只有第二个真实消费者出现后才抽成独立 npm 包。
