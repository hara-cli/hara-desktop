...
可以从 Hara 标识中的熊形轮廓建立拟人化角色母版，但角色数量与生成入口分阶段交付：

- 第一批只做 3 个轮廓差异明显的官方角色：默认 Hara、工程建造、研究探索。验证小尺寸
  可读性、9 个标准状态、16 个观察方向和透明窗口性能后，再扩展到约 7 个。
- 七个建议主题为：Hara、工程、研究、设计、Office、审阅守护、沟通交付。它们是皮肤与
  个性主题，不是假 Agent，也不代表某项能力一直在线。
- Image 模型只负责统一母版与逐行动画源图；最终 8×11 v2 atlas 必须经过确定性裁切、
  帧边界、方向语义、连续性、透明背景和实际显示尺寸 QA，不能让模型直接交付整张 atlas。
- Desktop 只内置默认 Hara 和最多两个精选角色；其余官方角色从 Hara Market 按需安装，
  避免七套动画永久增加安装包和更新带宽。
...
- 发布 3 个通过全套 v2 QA 的官方角色，再依据安装率与留存扩到约 7 个。
