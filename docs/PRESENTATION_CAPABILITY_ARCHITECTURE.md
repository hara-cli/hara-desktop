...
> 决策日期：2026-07-18
> 状态：架构确定，Desktop 第一阶段入口已开始实施；独立 `hara-presentation` 包尚未创建。
...
规划中的独立能力包第一版只承诺（当前 Desktop 卡片只是任务入口，不等于这些 exporter 已可用）：
...
建议独立仓库和 npm 包：`@nanhara/hara-presentation`。
