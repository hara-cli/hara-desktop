...
“能力”是用户要完成的结果，“插件包”是安装、版本、权限与回滚边界，“技能”是能力包可以贡献的
Markdown 工作方法。三者不能作为同级入口重复展示。

- 设置侧栏只保留一个“能力中心”；个人、项目和能力包提供的技能统一放在其中的“我的技能”；
- 创建技能仍通过安全的对话式创建器完成，预览和确认前不写文件；
- 能力包详情负责展示 Skill、Agent、Tool/MCP、Panel、Template 与 Policy 等贡献项；
- “侧栏模块”只管理稳定工作入口的显示和顺序，不承担能力发现或安装；
- 聊天、工作台、Office 和右侧扩展屏负责使用能力，能力中心只负责发现与管理。

P0 继续组合现有 `plugins.list` 与 `skills.list`，不为界面合并复制一套运行时。后续 manifest v2
再提供统一、带 owner 的 contribution 读取模型，并保持包安装、连接器授权和 Panel 运行状态相互独立。
