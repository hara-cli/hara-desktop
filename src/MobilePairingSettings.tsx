...
import type {
  ...
  MobileDesktopAuthorizationInvitation,
  MobileDesktopAuthorizationSnapshot,
...
const COPY = {
  en: {
    ...
    authorizationApproved: "Desktop signed in",
    authorizationApprovedBody:
      "This Desktop joined your Hara account and paired with the phone that approved it. No Sessions are shared until you allow them below.",
    authorizationCode: "One-time sign-in code",
    authorizationCreate: "Sign in with Hara Mobile",
    authorizationCreating: "Creating sign-in QR…",
    authorizationDescription:
      "Already signed in on your phone? Scan a five-minute invitation to approve this Desktop and join it to the same Hara account.",
    authorizationInstructions:
      "Open Hara Mobile → Pair a computer → Scan QR. Review this Desktop name and key ending before approval.",
    authorizationInvalid: "Hara returned an invalid Desktop authorization.",
    authorizationPending: "Waiting for approval in Hara Mobile",
    authorizationQrAlt: "Short-lived Hara Desktop sign-in QR code",
    authorizationRetry: "Create another QR",
  ...
  zh: {
    ...
    authorizationApproved: "Desktop 已登录",
    authorizationApprovedBody:
      "这台 Desktop 已加入你的 Hara 账号，并与批准它的手机完成配对。你在下方明确开放前，不会共享任何会话。",
    authorizationCode: "一次性登录码",
    authorizationCreate: "使用 Hara Mobile 登录",
    authorizationCreating: "正在生成登录二维码…",
    authorizationDescription:
      "手机已经登录 Hara？扫描一份五分钟有效的邀请，在手机上批准这台 Desktop 加入同一个 Hara 账号。",
    authorizationInstructions:
      "打开 Hara Mobile → 配对电脑 → 扫描二维码。批准前请核对 Desktop 名称和设备密钥尾号。",
    authorizationInvalid: "Hara 返回的 Desktop 授权信息无效。",
    authorizationPending: "等待在 Hara Mobile 中批准",
    authorizationQrAlt: "短时有效的 Hara Desktop 登录二维码",
    authorizationRetry: "重新生成二维码",
    approve: "允许这台手机",
    approved: "手机已配对",
    approvedBody:
...
function validInvitation(value: MobilePairingInvitation): boolean {
  ...
  return value.protocolVersion === 1
    ...
    && /^HARA_(?!AUTH_)[A-Za-z0-9_-]{11,123}$/u.test(value.pairingCode)
...
function validDesktopAuthorizationInvitation(
  value: MobileDesktopAuthorizationInvitation,
): boolean {
  const expectedPayload = `hara://authorize-desktop?v=1&region=${value.accountRegion}&code=${value.authorizationCode}&expires=${Math.floor(value.expiresAt / 1_000)}`;
  return value.protocolVersion === 1
    && (value.accountRegion === "cn" || value.accountRegion === "global")
    && /^[A-Za-z0-9._~-]{1,160}$/u.test(value.challengeId)
    && /^HARA_AUTH_[A-Za-z0-9_-]{22,118}$/u.test(value.authorizationCode)
    && value.qrPayload === expectedPayload
    && Number.isSafeInteger(value.expiresAt)
    && value.expiresAt > Date.now()
    ...
    && value.desktop.label.length > 0
    && value.desktop.label.length <= 120
    && ["macos", "windows", "linux"].includes(value.desktop.platform)
    && /^[a-f0-9]{64}$/u.test(value.desktop.publicKeyThumbprint);
}

function validDesktopAuthorizationSnapshot(
  value: MobileDesktopAuthorizationSnapshot,
  invitation: MobileDesktopAuthorizationInvitation,
): boolean {
  if (
    value.protocolVersion !== 1
    || typeof value.signedIn !== "boolean"
    || !["pending", "approved", "consumed", "expired", "cancelled", "missing"].includes(value.state)
  ) return false;
  if (value.signedIn) {
    return value.state === "consumed"
      ...
      && value.account.displayName.length > 0
      && (value.account.region === "cn" || value.account.region === "global");
  }
  if (
    value.challengeId !== invitation.challengeId
    || value.account !== null
    || !Number.isSafeInteger(value.expiresAt)
    || value.expiresAt !== invitation.expiresAt
    || value.state === "missing"
  ) return false;
  if (value.state === "expired") return value.desktop === null;
  return value.desktop?.label === invitation.desktop.label
    && value.desktop.platform === invitation.desktop.platform
    && value.desktop.publicKeyThumbprint === invitation.desktop.publicKeyThumbprint;
}
...
export function MobilePairingSettings({ client, locale }: Props) {
  ...
  const [authorization, setAuthorization] = useState<MobileDesktopAuthorizationInvitation | null>(null);
  const [authorizationStatus, setAuthorizationStatus] = useState<MobileDesktopAuthorizationSnapshot | null>(null);
  ...
  const [busy, setBusy] = useState<"authorize" | "create" | "approve" | "reject" | "">("");
  ...
  const statusSupported = Boolean(client && client.supports("mobile.status"));
  const authorizationSupported = Boolean(
    client
    && client.supportsFeature("mobile.desktop-authorization.qr.v1")
    && client.supports("mobile.authorization.create")
    && client.supports("mobile.authorization.status"),
  );
  const pairingSupported = Boolean(
    client
  ...
  const refreshAccount = useCallback(async () => {
    if (!client || !statusSupported || currentClientRef.current !== client) return;
  ...
  const qrPayload = authorization?.qrPayload ?? invitation?.qrPayload ?? "";
  ...
  useEffect(() => {
    ...
    let disposed = false;
    void import("qrcode")
      .then((module) => module.toDataURL(qrPayload, {
  ...
  const authorizationChallengeId = authorization?.challengeId ?? "";
  const authorizationExpiresAt = authorization?.expiresAt ?? 0;
  const authorizationTerminal = Boolean(
    authorizationStatus?.signedIn
    || authorizationStatus?.state === "expired"
    || authorizationStatus?.state === "cancelled"
    || authorizationStatus?.state === "missing",
  );
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (observedAt >= authorizationExpiresAt) return;
      try {
        const next = await client.mobileDesktopAuthorizationStatus();
        if (
          disposed
          || currentClientRef.current !== client
          || !validDesktopAuthorizationSnapshot(next, authorization)
        ) {
          if (!disposed && currentClientRef.current === client) {
            setError(copy.authorizationInvalid);
          }
          return;
        }
        setAuthorizationStatus(next);
        if (next.signedIn) await refreshAccount();
      } catch (reason) {
        if (!disposed && currentClientRef.current === client && Date.now() < authorizationExpiresAt) {
          setError(safeError(reason));
        }
      } finally {
        if (!disposed && currentClientRef.current === client && Date.now() < authorizationExpiresAt) {
  ...
  const authorizationSecondsLeft = authorization
    ? Math.max(0, Math.ceil((authorization.expiresAt - now) / 1_000))
    : 0;
  const authorizationActive = Boolean(
    authorization
    && authorizationSecondsLeft > 0
    && !authorizationStatus?.signedIn
    && (authorizationStatus?.state ?? authorization.state) === "pending",
  );
  const desktopKeySuffix = authorization?.desktop.publicKeyThumbprint.slice(-8).toUpperCase() ?? "";

  const createAuthorization = async () => {
    if (!client || busy || !authorizationSupported) return;
    const authorizationClient = client;
    setBusy("authorize");
    setError("");
    setCopied(false);
    setAuthorization(null);
    ...
    setQrDataUrl("");
    try {
      const created = await authorizationClient.createMobileDesktopAuthorization();
      if (currentClientRef.current !== authorizationClient) return;
      if (!validDesktopAuthorizationInvitation(created)) {
        throw new Error(copy.authorizationInvalid);
      }
      setNow(Date.now());
      setAuthorization(created);
    } catch (reason) {
      if (currentClientRef.current === authorizationClient) setError(safeError(reason));
    } finally {
      if (currentClientRef.current === authorizationClient) setBusy("");
    }
  };
  ...
  return (
    <>
      <SettingsCard
        title={copy.title}
        description={account?.signedIn ? copy.description : copy.authorizationDescription}
        aside={accountBadge}
      >
        <SettingsItem
          title={copy.account}
          description={accountChecked
            ? account?.account?.displayName ?? (
              authorizationSupported ? copy.authorizationDescription : copy.accountMissingBody
            )
        ...
        {!accountChecked ? null : !account?.signedIn && authorizationSupported ? (
          <div className="mobile-pairing-action-row">
            <button
              type="button"
              onClick={() => void createAuthorization()}
              disabled={Boolean(busy) || authorizationActive}
            >
              {busy === "authorize"
                ? copy.authorizationCreating
                : authorizationActive
                  ? copy.authorizationPending
                  : authorization
                    ? copy.authorizationRetry
                    : copy.authorizationCreate}
            </button>
          </div>
        ) : !account?.signedIn ? (
          <SettingsNotice
          ...
          <SettingsNotice tone="warning" title={copy.title}>{copy.unsupported}</SettingsNotice>
        )}

        {error ? <SettingsNotice tone="error" title={copy.title}>{error}</SettingsNotice> : null}

        {!account?.signedIn && authorization ? (
          <div className="mobile-pairing-ritual" aria-live="polite">
            <div className="mobile-pairing-qr-panel">
              <div className="mobile-pairing-qr-frame">
                {qrDataUrl ? <img src={qrDataUrl} alt={copy.authorizationQrAlt} /> : <span aria-hidden />}
              </div>
              <strong>
                {authorizationSecondsLeft > 0
                  ? replaceSeconds(copy.expires, authorizationSecondsLeft)
                  : copy.expired}
              </strong>
              <small>{copy.authorizationInstructions}</small>
            </div>

            <div className="mobile-pairing-review-panel">
              <div className="mobile-pairing-code-label">{copy.authorizationCode}</div>
              <div className="mobile-pairing-code-row">
                <code>{authorization.authorizationCode}</code>
                <button
                  type="button"
                  className="ghost"
                  onClick={() => void copyCode(authorization.authorizationCode)}
                >
              ...
              <div className="mobile-pairing-device-review">
                <div className="mobile-pairing-device-mark" aria-hidden>PC</div>
                <div>
                  <small>{copy.accountMissing}</small>
                  <strong>{authorization.desktop.label}</strong>
                  <span>
                    {authorization.desktop.platform.toUpperCase()} · {copy.fingerprint.replace("{{suffix}}", desktopKeySuffix)}
                  </span>
                </div>
              </div>

              {authorizationStatus?.signedIn ? (
                <SettingsNotice tone="success" title={copy.authorizationApproved}>
                  {copy.authorizationApprovedBody}
                </SettingsNotice>
              ) : authorizationStatus?.state === "expired"
                || authorizationStatus?.state === "cancelled"
                || !authorizationActive ? (
          ...
          <div className="mobile-pairing-ritual" aria-live="polite">
            ...
            <div className="mobile-pairing-review-panel">
              ...
              <div className="mobile-pairing-code-row">
                ...
                <button type="button" className="ghost" onClick={() => void copyCode(invitation.pairingCode)}>
                  {copied ? copy.copied : copy.copy}
                </button>
              </div>
