...
# Tauri's updater configuration is generated by the Rust build script. The protected signing
# runner is persistent, so a clean source checkout alone cannot prove that target-specific build
# output is fresh. Invalidate this package's generated output for the architecture being signed
# while retaining dependency caches.
echo "▸ invalidating architecture-specific desktop configuration cache"
